import { Autocomplete, Button, Checkbox, Chip, CircularProgress, FormControl, Grid, InputLabel, MenuItem, Popper, Select, TextField } from '@mui/material'
...
const Filters = ({ platforms,formStates, campaingData, isPending }) => {
    ...
    const { isValid, dirtyFields, errors, touchedFields,control, onSubmit, values, handleSubmit, setValue } = formStates;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
              <Grid container mt={3} px={2} gap={3} justifyContent={"center"}>
                          ...
                          render={({ field }) => (
                              <Autocomplete
                                  ...
                                  onChange={(e, value) => {
                                      ...
                                      if (value?.id === "all") {
                                        setValue("campaigns",[{ id: "all", name: "All" }], { shouldDirty: true, shouldValidate: true })
                                      }
                                  }}
                                  renderInput={(params) => (
                                      <TextField
                      ...
                      <Controller
                          ...
                          render={({ field }) => (
                              <Autocomplete
                                  {...field}
                                  multiple
                                  options={[{ id: "all", name: "All" }, ...get(campaignList, "result.data", [])]}
                                  ...
                                  renderOption={(props, option, { selected }) => (
                                      <li {...props}>
                                          <Checkbox
                                              checked={selected || field.value.some((val) => val.id === option.id)}
                                              sx={{
                                    ...
                                    <Select
                                        ...
                                        onChange={(e) => {
                                            ...
                                            if (e.target.value === "live") {
                                                setValue("dateRange", initialState?.dateRange, { shouldValidate: true });
                                            }
                                        }}
                                    >
                  ...
                  {values?.filterType === "dateRange" ? <Grid item xs={12} sm={5} alignSelf={"flex-start"}>
                        <Controller
                            name="dateRange"
                            control={control}
                            render={({ field }) => (
                        ...
                        />
                    </Grid>:<Grid item xs={12} sm={5}></Grid>}
                  <Grid item xs={12} textAlign={"center"}>
                      <Button
                          variant="contained"
                          className="p-bg-color"
                          type="submit"
                          disabled={isEmpty(dirtyFields) || !isValid || isPending || !isEmpty(errors)}
                      >
